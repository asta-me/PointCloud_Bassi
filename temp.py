...
np.random.seed(1)
O = Body(3*radius, 27*mass, pos=np.array((0.,0.,0.)))
L1 = 1.1
L2 = 1.1
vel1 = np.array((0.,0.02,0.))
vel2 = np.array((0.,0.1,0.))
H1 = Body(radius, mass, pos=np.array(( L1*np.sin(angle/2),L1*np.cos(angle/2),0.)),vel = vel1)
H2 = Body(radius, mass, pos=np.array((-L2*np.sin(angle/2),L2*np.cos(angle/2),0.)),vel =vel2)
